...
window.CS.reliability = (function () {
  ...
  const CFG = {
    MAX_SINGLE_ROBOT_FUEL:    250,   // Physical plausibility cap per individual field (balls per match)
    MAX_COMBINED_FUEL:        200,   // Max plausible auto+teleop total for ONE robot in one match
    ...
    // Intake rate → maximum plausible AUTO fuel for that rate bracket.
    // Auto period = 15 s. Cap = rate_max × 15 s + small buffer.
    // Used for single-submission impossibility detection (no peer comparison needed).
    AUTO_FUEL_CAP_BY_RATE: {
      "0-3":   50,   // ≤3 pieces/sec × 15 s ≈ 45 max
      "3.1-6": 100,  // ≤6 pieces/sec × 15 s ≈ 90 max
      "6.1+":  180,  // ≤12 pieces/sec × 15 s ≈ 180 max (very generous)
    },
  };
  ...
  /**
   * Check a SINGLE submission for physically impossible or internally inconsistent values.
   * Works with any sample size — requires NO peer comparisons.
   *
   * Two checks:
   *   (a) Combined fuel (auto + teleop) vs single-robot match cap
   *   (b) Auto fuel vs the reported intake rate bracket — the rate limits how much
   *       a robot could possibly score in the 15-second auto period
   *
   * @param {object} sub — must have .answers
   * @returns {{ impossible: boolean, reasons: string[] }}
   */
  function plausibilityCheckSubmission(sub) {
    const a       = sub.answers || {};
    const reasons = [];

    const autoFuel = Number(a.auto_fuel_scored  ?? 0);
    const teleFuel = Number(a.teleop_fuel_scored ?? 0);
    const combined = autoFuel + teleFuel;

    // (a) Combined cap — one robot cannot score more than this total in a match
    if (combined > CFG.MAX_COMBINED_FUEL) {
      reasons.push(
        `Combined fuel ${combined} exceeds single-robot match cap ${CFG.MAX_COMBINED_FUEL}`
      );
    }

    // (b) Intake rate vs auto fuel: auto period is 15 s; the bracket limits peak throughput
    const autoRate    = a.auto_intake_rate;
    const autoRateCap = autoRate ? CFG.AUTO_FUEL_CAP_BY_RATE[autoRate] : undefined;
    if (autoRateCap !== undefined && autoFuel > autoRateCap) {
      reasons.push(
        `Auto fuel ${autoFuel} is impossible for intake rate "${autoRate}" ` +
        `(max ~${autoRateCap} at that rate in a 15 s auto period)`
      );
    }
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  // LIVE RELIABILITY COMPUTATION FROM SUBMISSIONS
  // Given all submissions for a single team, detect outlier submissions and
  // compute a reliability score (0–1) for each scout who contributed.
  //
  // Outlier logic: for each key numeric field, run Modified Z-Score across ALL
  // submissions. A submission is an outlier if ANY key field is flagged.
  // Scout score = (non-outlier submissions) / (total submissions), floored at 0.2.
  //
  // This is called from team.html after fetching all subs for a team,
  // and the results are written back to Supabase.
  // ─────────────────────────────────────────────────────────────────────────────

  /**
   * @param {object[]} subs — each must have .id, .owner_id, .answers
   * @param {string[]} [keyFields] — which answer fields to use for outlier detection
   * @returns {{
   *   outlierIds: Set<string>,
   *   cleanSubs: object[],
   *   scoutReliabilities: Map<string, {score:number, outlierCount:number, totalCount:number}>
   * }}
   */
  function computeSubmissionReliabilities(subs, keyFields = ["auto_fuel_scored", "teleop_fuel_scored"]) {
    const outlierFlags = new Map(); // id → boolean
    subs.forEach(s => outlierFlags.set(s.id, false));

    // ── Pre-pass: plausibility check (works with ANY sample size, even 1) ──────
    // Flags submissions whose values are physically impossible regardless of
    // how many other submissions exist for the same team.
    for (const sub of subs) {
      const check = plausibilityCheckSubmission(sub);
      if (check.impossible) {
        outlierFlags.set(sub.id, true);
        console.warn(
          `[CS.reliability] Impossible submission flagged (id=${sub.id}):`,
          check.reasons.join("; ")
        );
      }
    }

    // ── Modified Z-Score outlier detection (requires ≥3 submissions per field) ─
    for (const field of keyFields) {
      const validSubs = subs.filter(s => s.answers?.[field] != null);
      if (validSubs.length < 3) continue; // not enough data for meaningful outlier detection

      const values = validSubs.map(s => Number(s.answers[field]));
      const { outlierMask } = removeOutliersModifiedZ(values);

      validSubs.forEach((s, i) => {
        if (outlierMask[i]) outlierFlags.set(s.id, true);
      });
    }

    // Per-scout stats
    const scoutStats = new Map(); // owner_id → { total, outliers }
    subs.forEach(s => {
      if (!s.owner_id) return;
      if (!scoutStats.has(s.owner_id)) scoutStats.set(s.owner_id, { total: 0, outliers: 0 });
      const st = scoutStats.get(s.owner_id);
      st.total++;
      if (outlierFlags.get(s.id)) st.outliers++;
    });

    const scoutReliabilities = new Map();
    for (const [ownerId, st] of scoutStats) {
      const score = st.total === 0 ? 1.0 : Math.max(0.2, (st.total - st.outliers) / st.total);
      scoutReliabilities.set(ownerId, { score, outlierCount: st.outliers, totalCount: st.total });
    }

    const outlierIds = new Set([...outlierFlags.entries()].filter(([, v]) => v).map(([k]) => k));
    const cleanSubs  = subs.filter(s => !outlierIds.has(s.id));

    return { outlierIds, cleanSubs, scoutReliabilities };
  }
  ...
  return {
    // Config
    CFG,
    // Live reliability from submissions
    computeSubmissionReliabilities,
    // Single-submission plausibility check (works with any sample size)
    plausibilityCheckSubmission,
